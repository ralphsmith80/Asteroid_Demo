...
//		p_maximizeProcessor = false;
		...
		p_frameCount_real = 0;
		p_frameRate_real = 0;
		p_ambientColor = Color4fMake(0.0, 0.0, 0.0, 1.0); //black
//		p_windowHandle = 0;
		p_pauseMode = false;
		...
		//window handle must be set later on
//		this->setWindowHandle(0);
	...
	Engine::~Engine()
	{
		...
		iter = this->p_entities.begin();
		for (;iter != this->p_entities.end(); ++iter) {
			delete *iter;
		}
...
//		if (this->p_device == NULL) return 0;
		...
		//set color depth
		this->p_colordepth = colordepth;
		
		//set fullscreen mode
		this->p_fullscreen = fullscreen;
		
		//clear the backbuffer to black
		this->ClearScene(Color4fMake(0.0, 0.0, 0.0, 1.0));
...
//		this->p_device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &this->p_backbuffer);
	...
	void Engine::ClearScene(Color4f color)
	{
		glClearColor(color.red, color.green, color.blue, color.alpha);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	}
	
	void Engine::SetAmbient(Color4f colorvalue)
	{
